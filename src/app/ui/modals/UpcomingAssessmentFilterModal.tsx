...
export default function UpcomingAssessmentFilterModal({ onApply, onClose }: Props) {
  const [type, setType] = useState<string[]>(['MCQ+Coding']);
  const [scheduled, setScheduled] = useState<string[]>(['This Week']);
            ...
            {['All', 'Tomorrow', 'This Week', 'Next Week'].map((item) => (
              ...
              >
                <input
                  ...
                  onChange={() => toggleValue(item, scheduled, setScheduled)}
                  className="peer relative h-4 w-4 appearance-none rounded border border-gray-300
                  checked:border-transparent
                  checked:bg-[linear-gradient(90deg,#904BFF_0%,#C053C2_100%)]
                  checked:after:content-['✔']
