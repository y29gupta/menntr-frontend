...
export default function UpcomingAssessmentFilterModal({ initialValues, onApply, onClose }: Props) {
  const [type, setType] = useState<string[]>(initialValues.type);
  const [scheduled, setScheduled] = useState<string[]>(initialValues.scheduled);

  useEffect(() => {
    setType(initialValues.type);
    setScheduled(initialValues.scheduled);
  }, [initialValues]);
      ...
      <div className="absolute left-0 top-full z-50 mt-2 w-[360px] rounded-2xl bg-white p-5 shadow-[0_12px_40px_rgba(0,0,0,0.15)]">
        <div className="flex text-sm">
          ...
          <div className="flex-1 pl-4">
            <p className="mb-3 font-medium">Scheduled</p>

            {['All', 'Tomorrow', 'This Week', 'Next Week'].map((item) => (
              <label
                key={item}
                className="mb-3 flex cursor-pointer items-center gap-3 text-gray-700"
              >
                <input
                  type="checkbox"
                  checked={scheduled.includes(item)}
                  onChange={() => toggleSingle(item, setScheduled)}
                  className="peer relative h-4 w-4 appearance-none rounded border border-gray-300
                  checked:border-transparent
                  checked:bg-[linear-gradient(90deg,#904BFF_0%,#C053C2_100%)]
                  checked:after:content-['✔']
        ...
        <div className="flex gap-3">
          <button
            onClick={() => onApply({ type, scheduled })}
