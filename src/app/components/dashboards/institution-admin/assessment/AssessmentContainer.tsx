...
export default function AssessmentContainer() {
  return (
    <EvaluationContainer
      ...
      api={{
        getList: assessmentApi.getAssessmentList,
      }}
      statuses={{
        active: 'active',
        draft: 'draft',
      ...
      Header={(props) => (
        <AssessmentHeader
          {...props}
          title="Assessments"
          description="Create, manage and track institution assessments"
          createLabel="Create Assessment"
          Icon={AssessmentIcon}
          createPermission={PERMISSIONS.ASSESSMENT.ASSESSMENTS.CREATE}
        />
      )}
      Filters={(props) => <AssessmentFilters {...props} placeholder="Search for assessments" />}
      ActiveComponent={ActiveAssessments}
      DraftComponent={DraftAssessments}
      CompletedComponent={ActiveAssessments}
    />
  );
}
